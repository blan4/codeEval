...
OUTPUT SAMPLE: Print the result in the following way.
135 0 54
40
13 16 225 14 120 10
DOC
...
File.readlines(ARGV.first).each do |line|
  ...
  left = vectors.first.split(' ').map(&:to_i)
  right = vectors.last.split(' ').map(&:to_i)
  res = left.map.with_index do |element, index|
    element * right[index]
  end
  puts res.join(' ')
end