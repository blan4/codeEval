...
INPUT SAMPLE: The first argument is a path to a filename. Each line in the file contains the list of cities and distances to them, comma delimited, from the starting point of your trip. You start your trip at point 0. The cities with their distances are separated by semicolon. E.g.
...
OUTPUT SAMPLE: Print out the distance from the starting point to the nearest city, and the distances between two nearest cities separated by comma, in order they appear on the route. E.g.
...
File.readlines(ARGV.last).each do |line|
  ...
  distances = cities.map do |city| 
    city.split(',').last.to_i
  end.sort
  res = []
  res << distances[0]
  distances.each_cons(2) do |pair| 
    res << pair[1] - pair[0]
  end
  puts res.join ','
end